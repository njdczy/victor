...
class CustomExporter extends AbstractExporter
{
    ...
    public function export()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $export[] = array_key_exists($value['company'],$company_values_array)?$company_values_array[$value['company']]:'';                   //客户
            $export[] = array_key_exists($value['hotel'],$hotel_values_array)?$hotel_values_array[$value['hotel']]:'';                   //入住饭店
            $export[] = $value['has_attend'] ? '是' : '否';                   //是否参加过会议
            $export[] = array_key_exists($value['salesman_id'],$salesman_values_array)?$salesman_values_array[$value['salesman_id']]:'';                   //业务员
            $export[] = array_key_exists($value['regional_manager_id'],$manager_values_array)?$manager_values_array[$value['regional_manager_id']]:'';                   //区域经理
